...
p('/backend/package/api/bitcoin/bitcoin-api.js',
  /const innerTx = \\(await this\\.\\$getRawTransaction\\(vin\\.txid, false, false\\)\\);\\s*vin\\.prevout = innerTx\\.vout\\[vin\\.vout\\];\\s*transaction_utils_1\\.default\\.addInnerScriptsToVin\\(vin\\);/,
  'try { const innerTx = (await this.$getRawTransaction(vin.txid, false, false)); vin.prevout = innerTx && innerTx.vout ? innerTx.vout[vin.vout] : null; transaction_utils_1.default.addInnerScriptsToVin(vin); } catch (ePrev) { console.warn("[address-txs] prevout skip", vin && vin.txid, (ePrev && ePrev.message) || ePrev); }');
p('/backend/package/api/bitcoin/bitcoin-api.js',
