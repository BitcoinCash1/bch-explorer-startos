...
export const setDependencies = sdk.setupDependencies(async ({ effects }) => {
  ...
  if (store?.nodeConfirmed) {
    if (node === 'bitcoincashd') {
      ...
      // database mode keeps the transaction data the explorer looks up.
      await sdk.action.createTask(
        effects,
        'knuth-bch',
        knuthAutoconfig,
        'critical',
        {
          ...
          when: { condition: 'input-not-matches', once: false },
          reason: i18n(
            'BCH Explorer reads the chain from Knuth over JSON-RPC, which needs JSON-RPC enabled and the full database mode',
          ),
        },
      )
    }
